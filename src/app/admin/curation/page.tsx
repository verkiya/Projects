...
export default function AdminPage() {
  ...
  const handleSync = async (e?: React.FormEvent) => {
    ...
    setIsSyncing(true);
    setSyncProgress(0);
    setTotalVideos(fetchedLikedVideos.length);
    try {
      let successCount = 0;
      const createdCategories = new Set<string>();

      for (let i = 0; i < fetchedLikedVideos.length; i++) {
        const video = fetchedLikedVideos[i];
                      ...
                      {!fetchedLikedVideos ? (
                        ...
                        <div className="flex flex-col gap-6 w-full">
                          ...
                          {isSyncing && (
                            <div className="w-full bg-white/5 rounded-full h-2 mt-4 overflow-hidden">
                              <div
                                ...
                                style={{ width: `${(syncProgress / totalVideos) * 100}%` }}
                              />
                            </div>
                          )}
                          ...
                          <div className="flex gap-4 pt-4 border-t border-white/10">
                            ...
                            >
                              {isSyncing ? `Syncing ${syncProgress}/${totalVideos}...` : "Sync Selected to Database"}
                            </Button>
                          </div>
                        </div>
                      )}
